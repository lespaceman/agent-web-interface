...
1. **Timing gap:** Elements that appear *after* DOM stabilization completes are missed entirely
...
**Key observation:** Toast text is **only** captured when the toast *leaves* the DOM (`<disappeared>`) during a subsequent action's observation window. It is never captured on appearance.
...
const essentialStructuralRoles = new Set([
  ...
  'alert', 'status', 'log', 'timer', 'tooltip', 'progressbar'
]);
```
